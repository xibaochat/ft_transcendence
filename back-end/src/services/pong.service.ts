...
export class PongService {
	...
	async joinPongRoom(client: Socket, server: Server, userId: string, roomId: string) {
		...
		if (!game) {
			...
			let updatedUser = await this.userService.updateUser( { status: game.roomId }, firstPlayer.id.toString());
			updatedUser = await this.userService.updateUser( { status: game.roomId }, secondPlayer.id.toString());
